...
// To compile we need to ensure that symbols referred in this file are
// found. This requires two steps: (1) import symbols explicitly (2)
// ensure clojure.lang symbols are on the classpath:
//
// (1) Since we changed to our own fullmeta package (essentially Java
// version of a namespace) we must import classes we need explicitly.
// MultiFn.java that is part of Clojure source doesn't need to do that
// because all those symbols are in the same clojure.lang package.
//
// (2) ensure the Java compiler can find clojure.lang symbols:
//
// javac -cp `clj -Spath`:/Users/russki/Code/clojure/src/jvm/clojure/lang clj/src/fullmeta/*.java
//
// Turns out those symbols are already on the classpath, so we don't
// need to add clojure/lang folder at all, since those would have been
...
static final Var isa = RT.var("fullmeta.dispatch", "isa?");
static final Var parents = RT.var("fullmeta.dispatch", "parents");
...
private IPersistentMap resetCache() {
 ...
 public IFn getMethod(Object dispatchVal) {
	 // NOTE(fullmeta) this is to catch race condition that may have cached stale method see
	 // comment in findAndCacheBestMethod
...
private IFn findAndCacheBestMethod(Object dispatchVal) {
	...
	try
		...
		if( mt == methodTable &&
		    ...
		    ch == cachedHierarchy &&
			cachedHierarchy == hierarchy.deref())
			// NOTE(fullmeta) this is subtle. There is a race here: we've computed our
			// effective method but between the hierarchy check above (deref) and
			// updating the cache below hierarchy may have changed so we potentially
			// cache stale method value. This is caught by performing the same check in
			// getMethod, see note there.
			{
			//place in cache
